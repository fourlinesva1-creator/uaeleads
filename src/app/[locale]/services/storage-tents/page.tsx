import { setRequestLocale } from 'next-intl/server';
import { Metadata } from 'next';
import Image from 'next/image';
import { Link } from '@/i18n/navigation';
import { routing } from '@/i18n/routing';
import { CheckCircle2, ArrowRight, ChevronDown, Warehouse, Package, Thermometer } from 'lucide-react';
import ServiceSchema from '@/components/seo/ServiceSchema';
import FAQSchema from '@/components/seo/FAQSchema';
import BreadcrumbSchema from '@/components/seo/BreadcrumbSchema';
import { storageTentHubContent } from '@/data/storage-tent-content';
import PricingNote from '@/components/ui/PricingNote';

type Props = {
    params: Promise<{ locale: string }>;
};

export async function generateStaticParams() {
    return routing.locales.map(locale => ({ locale }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const { locale } = await params;
    const isAr = locale === 'ar';
    return {
        title: isAr
            ? 'تأجير خيام التخزين في الإمارات | خيام المستودعات والصناعة | تنت ناو'
            : 'Storage Tent Rental UAE | Warehouse & Industrial Tents | Tent Now',
        description: isAr
            ? 'تأجير خيام التخزين في جميع أنحاء الإمارات — خيام مستودعات، كلير سبان وهياكل صناعية. معتمدة من الدفاع المدني. تخدم دبي وأبوظبي والشارقة وجميع الإمارات السبع.'
            : 'Storage tent rental across UAE — warehouse tents, clear span & industrial structures. Civil Defence approved. Serving Dubai, Abu Dhabi, Sharjah & all 7 emirates.',
        alternates: {
            canonical: `https://www.tentnow.ae/${locale}/services/storage-tents`,
            languages: {
                'en': 'https://www.tentnow.ae/en/services/storage-tents',
                'ar': 'https://www.tentnow.ae/ar/services/storage-tents',
                'x-default': 'https://www.tentnow.ae/en/services/storage-tents',
            },
        },
        openGraph: {
            title: 'Storage Tent Rental UAE | Warehouse & Industrial Tents | Tent Now',
            description: 'Storage tent rental across UAE — warehouse tents, clear span & industrial structures. Civil Defence approved. Serving Dubai, Abu Dhabi, Sharjah & all 7 emirates.',
            images: [{ url: 'https://www.tentnow.ae/images/storage-tents/storage-tent-hero.jpg' }],
        },
    };
}

const tentTypeIcons = [Warehouse, Package, Thermometer];

export default async function StorageTentsPage({ params }: Props) {
    const { locale } = await params;
    setRequestLocale(locale);
    const c = storageTentHubContent[locale] ?? storageTentHubContent['en'];
    const pageUrl = `https://www.tentnow.ae/${locale}/services/storage-tents`;
    const isRtl = locale === 'ar';

    return (
        <div className="bg-[#101622]" dir={isRtl ? 'rtl' : 'ltr'}>
            <ServiceSchema
                name="Industrial Storage Tent Rental UAE"
                description="Civil Defence approved clear span, warehouse frame, and industrial storage tent solutions for construction, logistics, oil & gas, and manufacturing projects across the UAE."
                url={pageUrl}
                image="https://www.tentnow.ae/images/storage-tents/storage-tent-hero.jpg"
            />
            <FAQSchema items={c.faqs} />
            <BreadcrumbSchema
                locale={locale}
                items={[
                    { name: isRtl ? 'الخدمات' : 'Services', href: '/services' },
                    { name: isRtl ? 'خيام التخزين' : 'Storage Tents', href: '/services/storage-tents' },
                ]}
            />

            {/* Hero */}
            <section className="relative pt-32 lg:pt-48 pb-28 overflow-hidden">
                <div className="absolute inset-0 z-0">
                    <Image
                        src="/images/storage-tents/storage-tent-hero.jpg"
                        alt="Industrial storage tent structures UAE"
                        fill
                        priority
                        className="object-cover opacity-20 scale-105"
                    />
                    <div className="absolute inset-0 bg-gradient-to-b from-[#101622]/80 via-[#101622] to-[#101622]" />
                </div>

                <div className="container-luxury relative z-10">
                    <div className="max-w-4xl animate-fade-in-up">
                        <div className="section-label mb-8">
                            <span>{c.heroLabel}</span>
                        </div>
                        <h1 className="text-4xl md:text-7xl font-display text-white mb-10 tracking-tight leading-[1.1]">
                            {c.heroTitle}
                        </h1>
                        <div className="w-24 h-1.5 bg-gold mb-12 shadow-[0_0_15px_rgba(212,175,55,0.3)]" />
                        <p className="text-xl md:text-2xl text-[#9da6b9] font-light leading-relaxed max-w-3xl">
                            {c.heroBody}
                        </p>
                        <div className="flex flex-wrap gap-4 mt-12">
                            <a
                                href="https://wa.me/971501826969"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="btn-gold-fill px-8 py-4 rounded-xl font-display inline-flex items-center gap-3"
                            >
                                {c.ctaPrimary}
                            </a>
                            <Link
                                href="/request-quote"
                                className="btn-secondary px-8 py-4 rounded-xl font-display inline-flex items-center gap-3"
                            >
                                {c.ctaSecondary}
                            </Link>
                        </div>
                    </div>
                </div>
            </section>

            {/* Market Context */}
            <section className="py-16 border-y border-[#1a212e]">
                <div className="container-luxury">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                        {c.stats.map(({ value, label }) => (
                            <div key={label} className="text-center">
                                <div className="text-3xl md:text-4xl font-display text-gold mb-2">{value}</div>
                                <div className="text-[#9da6b9] text-sm">{label}</div>
                            </div>
                        ))}
                    </div>
                </div>
            </section>

            {/* Why Storage Tents */}
            <section className="container-luxury py-24">
                <div className="max-w-3xl mb-16">
                    <h2 className="text-3xl md:text-4xl font-display text-white mb-6">{c.whyTitle}</h2>
                    <p className="text-[#9da6b9] text-lg leading-relaxed">{c.whyBody}</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {c.features.map((item) => (
                        <div
                            key={item.title}
                            className="p-8 bg-[#1a212e] border border-[#282e39] rounded-2xl hover:border-gold/30 transition-all group"
                        >
                            <CheckCircle2 className="text-gold mb-4 group-hover:scale-110 transition-transform" size={28} />
                            <h3 className="text-white text-lg font-bold mb-3">{item.title}</h3>
                            <p className="text-[#9da6b9] text-sm leading-relaxed">{item.body}</p>
                        </div>
                    ))}
                </div>
            </section>

            {/* Tent Type Cards */}
            <section className="py-24 border-t border-[#1a212e]">
                <div className="container-luxury">
                    <div className="max-w-2xl mb-16">
                        <div className="section-label mb-6">
                            <span>{c.typesLabel}</span>
                        </div>
                        <h2 className="text-3xl md:text-4xl font-display text-white mb-4">{c.typesTitle}</h2>
                        <p className="text-[#9da6b9]">{c.typesSubtitle}</p>
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        {c.tentTypes.map((tent, idx) => {
                            const Icon = tentTypeIcons[idx];
                            return (
                                <Link
                                    key={tent.href}
                                    href={tent.href}
                                    className="group bg-[#1a212e] border border-[#282e39] rounded-3xl overflow-hidden hover:border-gold/40 transition-all flex flex-col"
                                >
                                    <div className="relative aspect-video overflow-hidden">
                                        <Image
                                            src={`/images/storage-tents/storage-tent-${idx + 1}.jpeg`}
                                            alt={tent.title}
                                            fill
                                            className="object-cover group-hover:scale-105 transition-transform duration-700"
                                        />
                                        <div className="absolute inset-0 bg-gradient-to-t from-[#1a212e] via-transparent to-transparent" />
                                        <div className="absolute bottom-4 left-4 bg-gold/20 backdrop-blur-sm border border-gold/30 rounded-lg px-3 py-1.5">
                                            <span className="text-gold text-xs font-bold">{tent.stat}</span>
                                        </div>
                                    </div>
                                    <div className="p-8 flex-1 flex flex-col">
                                        <Icon className="text-gold mb-4" size={28} />
                                        <h3 className="text-white text-xl font-display font-bold mb-2 group-hover:text-gold transition-colors">
                                            {tent.title}
                                        </h3>
                                        <p className="text-gold/70 text-sm font-medium mb-4">{tent.subtitle}</p>
                                        <p className="text-[#9da6b9] text-sm leading-relaxed flex-1">{tent.body}</p>
                                        <div className="mt-6 flex items-center gap-2 text-gold text-sm font-bold">
                                            {isRtl ? 'اعرف المزيد' : 'Learn More'} <ArrowRight size={16} />
                                        </div>
                                    </div>
                                </Link>
                            );
                        })}
                    </div>
                </div>
            </section>

            {/* Industries Served */}
            <section className="py-24 bg-[#0d1219] border-t border-[#1a212e]">
                <div className="container-luxury">
                    <h2 className="text-3xl md:text-4xl font-display text-white mb-4">{c.industriesTitle}</h2>
                    <p className="text-[#9da6b9] mb-12 max-w-2xl">{c.industriesBody}</p>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        {c.industries.map((industry) => (
                            <div
                                key={industry}
                                className="p-4 bg-[#1a212e] border border-[#282e39] rounded-xl text-center hover:border-gold/30 transition-colors"
                            >
                                <span className="text-white text-sm font-medium">{industry}</span>
                            </div>
                        ))}
                    </div>
                </div>
            </section>

            {/* FAQ */}
            <section className="py-24 container-luxury border-t border-[#1a212e]">
                <PricingNote locale={locale} className="mb-10" />
                <h2 className="text-3xl md:text-4xl font-display text-white mb-12">{c.faqTitle}</h2>
                <div className="space-y-4 max-w-3xl">
                    {c.faqs.map((faq, idx) => (
                        <details
                            key={idx}
                            className="group bg-[#1a212e] border border-[#282e39] rounded-2xl overflow-hidden hover:border-gold/30 transition-colors"
                        >
                            <summary className="flex items-center justify-between gap-4 px-6 py-5 cursor-pointer list-none">
                                <h3 className="text-white font-display text-base md:text-lg leading-snug">{faq.q}</h3>
                                <ChevronDown className="text-gold shrink-0 transition-transform group-open:rotate-180" size={20} />
                            </summary>
                            <div className="px-6 pb-5">
                                <p className="text-[#9da6b9] leading-relaxed text-sm md:text-base">{faq.a}</p>
                            </div>
                        </details>
                    ))}
                </div>
            </section>

            {/* From Our Blog */}
            <section className="py-20 border-t border-[#1a212e]">
                <div className="container-luxury">
                    <h2 className="text-2xl md:text-3xl font-display text-[#D4AF37] mb-2">
                        {isRtl ? 'من مدونتنا' : 'From Our Blog'}
                    </h2>
                    <p className="text-[#9da6b9] text-sm mb-8">
                        {isRtl ? 'مقالات وأدلة ذات صلة بخيام التخزين.' : 'Guides and insights related to industrial storage tents.'}
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {[
                            { slug: 'industrial-storage-tent-rental-uae-2026', title: 'Industrial Storage Tent Rental UAE 2026' },
                            { slug: 'tarpaulin-vs-storage-tent-uae', title: 'Tarpaulin vs Storage Tent UAE: Which to Choose?' },
                            { slug: 'how-to-choose-tent-rental-company-uae', title: 'How to Choose a Tent Rental Company in UAE' },
                        ].map(({ slug, title }) => (
                            <Link
                                key={slug}
                                href={`/blog/${slug}`}
                                className="group flex flex-col justify-between p-6 bg-[#1a212e] border border-[#282e39] rounded-2xl hover:border-[#D4AF37]/40 transition-all"
                            >
                                <p className="text-white font-semibold leading-snug group-hover:text-[#D4AF37] transition-colors mb-4">
                                    {title}
                                </p>
                                <span className="text-[#D4AF37] text-sm font-bold flex items-center gap-1">
                                    {isRtl ? 'اقرأ المقال' : 'Read article'} <ArrowRight size={14} />
                                </span>
                            </Link>
                        ))}
                    </div>
                </div>
            </section>

            {/* CTA */}
            <section className="container-luxury pb-24">
                <div className="p-12 lg:p-16 bg-gold rounded-3xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-16 opacity-10 transform translate-x-8 -translate-y-8 group-hover:scale-110 transition-transform duration-700">
                        <ArrowRight size={300} strokeWidth={1} />
                    </div>
                    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                        <div className="max-w-xl text-center md:text-left">
                            <h2 className="text-3xl md:text-4xl font-display text-[#101622] font-bold mb-4">{c.ctaTitle}</h2>
                            <p className="text-[#101622]/80 font-medium">{c.ctaBody}</p>
                        </div>
                        <a
                            href="https://wa.me/971501826969"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="px-10 py-5 bg-[#101622] text-white rounded-xl font-bold tracking-widest uppercase hover:bg-[#1a212e] transition-all shadow-2xl whitespace-nowrap"
                        >
                            {c.ctaButton}
                        </a>
                    </div>
                </div>
            </section>
        </div>
    );
}
