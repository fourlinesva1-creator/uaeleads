...
export default async function ClearSpanTentsPage({ params }: Props) {
    const { locale } = await params;
    setRequestLocale(locale);
    const c = clearSpanContent[locale] ?? clearSpanContent['en'];
    const pageUrl = `https://www.tentnow.ae/${locale}/services/storage-tents/clear-span-tents`;
    const isRtl = locale === 'ar';

    return (
        <div className="bg-[#101622]" dir={isRtl ? 'rtl' : 'ltr'}>
            <ServiceSchema
                name="Clear Span Storage Tent Rental UAE"
                description="Column-free clear span aluminium frame tent structures from 3m to 50m+ width. Civil Defence approved, deployable in under 14 days across all UAE emirates."
                url={pageUrl}
                image="https://www.tentnow.ae/images/storage-tents/storage-tent-1.jpeg"
            />
            ...
            <BreadcrumbSchema
                ...
                items={[
                    ...
                    { name: isRtl ? 'خيام الامتداد الحر' : 'Clear Span Tents', href: '/services/storage-tents/clear-span-tents' },
                ]}
            />
            ...
            <section className="relative pt-32 lg:pt-48 pb-28 overflow-hidden">
                <div className="absolute inset-0 z-0">
                    <Image
                        src="/images/storage-tents/storage-tent-1.jpeg"
                        alt="Clear span tent structure UAE — column-free industrial storage"
                        fill
                        priority
            ...
            <section className="container-luxury py-20">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    ...
                    <div className="relative aspect-[4/3] rounded-3xl overflow-hidden border border-[#282e39]">
                        <Image src="/images/storage-tents/storage-tent-4.jpeg" alt="Clear span tent interior UAE" fill className="object-cover" />
                    </div>
                </div>
            </section>
            ...
            <section className="py-24 border-t border-[#1a212e]">
                <div className="container-luxury">
                    <h2 className="text-3xl md:text-4xl font-display text-white mb-4">{c.useCasesTitle}</h2>
                    <p className="text-[#9da6b9] mb-12 max-w-2xl">{c.useCasesSubtitle}</p>
            ...
            <section className="py-24 bg-[#0d1219] border-t border-[#1a212e]">
                <div className="container-luxury">
                    <h2 className="text-3xl font-display text-white mb-12">{c.specsTitle}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl">
                        {c.specs.map(({ label, value }) => (
                            <div key={label} className="flex items-center justify-between p-5 bg-[#1a212e] border border-[#282e39] rounded-xl">
                                <span className="text-[#9da6b9] text-sm">{label}</span>
                                <span className="text-white font-bold text-sm text-right">{value}</span>
                            </div>
                        ))}
                    </div>
                    <p className="text-[#9da6b9] text-sm mt-8 max-w-2xl">{c.specsNote}</p>
                </div>
            </section>
